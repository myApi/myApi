...
class MyapiViewSend extends JView {
    function display($tpl = null) {
		...
		$db = JFactory::getDBO();
		$query = "SELECT id FROM #__plugins WHERE element =".$db->quote('myApiSend');
		$db->setQuery($query);
		$id = $db->loadResult();
		...
		if(is_object($plugin)){
			...
			$paramsdefs = JPATH_SITE.DS.'plugins'.DS.'content'.DS.'myApiSend.xml';
			...
			global $mainframe;
			$mainframe->redirect('index.php?option=com_plugins&view=plugin&task=edit&cid='.$id,'The myApiSend plugin needs to be ENABLED before you can configure it.');	
		}
